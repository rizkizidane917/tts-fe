...
import { THistoryData } from "@/types/historyType";

const HistoryList = () => {
  const [openItem, setOpenItem] = useState<string | null>(null);
  const [historyData, setHistoryData] = useState<THistoryData[]>([]);
  const [playingItem, setPlayingItem] = useState<string | null>(null);
  const contentRefs = useRef<{
    [key: string]: HTMLDivElement | undefined | null;
  }>({});
  ...
  const handleReplay = (item: THistoryData) => {
    ...
    utterance.volume = item.volume / 100 || 1.0;
  ...
  const fetchQueryConversion = useCustomQuery({
    path: "/conversion",
    queryKey: ["conversion-list"],
  });

  useEffect(() => {
    if (fetchQueryConversion?.data) {
      const res = fetchQueryConversion?.data as THistoryData[];
      setHistoryData(res);
    }
  }, [fetchQueryConversion?.data]);
          ...
          <>
            {historyData?.length == 0 && (
              <div className="flex items-center justify-center py-10">
                <h4 className="text-lg"> - No History Data -</h4>
              </div>
            )}
            ...
            {historyData?.length !== 0 && (
              <div className="space-y-3 max-h-screen overflow-y-auto scroll-container">
                {historyData?.map((item, idx) => {
                  ...
                  const voicePreview = item?.voice?.split(" (")[0];
                  return (
                    <div
                      key={idx}
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-gray-500 font-semibold line-clamp-1">
                              {voicePreview}
                            </span>
                          </div>
